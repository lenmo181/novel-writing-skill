...
用法：
    python scripts/visualize.py "<项目根>"           # 生成 看板.html 并自动打开浏览器
    python scripts/visualize.py "<项目根>" --no-open # 只生成不打开
...
function chart(){
  ...
  if(!sv.some(v=>v!=null))return "<div class='empty'>暂无朱雀检测数据（跑 scripts/zhuque_check.py --book 后刷新）</div>";
...
function render(){
  ...
  document.getElementById("main").innerHTML=SECTIONS.map((s,i)=>`<section id="sec${i}" class="section"><h2>${s.name}</h2>${s.f()}</section>`).join("")+"<footer>本看板由 scripts/visualize.py 生成 · 数据源：书稿/ + mind/ · 改动后重跑即可刷新 · 阅读进度存在本机浏览器（localStorage），换电脑不跟随</footer>";
  go(0);archShow(0);
