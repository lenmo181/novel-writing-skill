...
（SKILL.md / references/ / templates/ / scripts/ / versions/），校验目标是否存在，
输出悬空清单。
...
# 技能包内可校验的引用前缀
CHECK_PREFIXES = ("references/", "templates/", "scripts/", "versions/")

# 项目侧运行时路径 / 外部能力：跳过不校验
SKIP_PREFIXES = ("mind/", "书稿/", "大纲/", "设定/", "封面/", "拆文库/",
                 ".story-review/", ".workbuddy/", "workspace/", "node_modules/")

# 被引用的候选路径：以 .md / .py 结尾的相对路径或 SKILL.md
REF_RE = re.compile(
    r"(?:SKILL\.md"
    r"|(?:references|templates|scripts|versions)/[^\s`\"'（）()\[\]【】《》<>、，。；：!?！？]+?\.(?:md|py)"
    r")")
